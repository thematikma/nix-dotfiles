...
local blocks = {
    ...
    oxwm.bar.block.shell({
        format = "{}",
        command = "playerctl metadata --format '{{artist}} - {{title}}' 2>/dev/null | cut -c1-50",
        interval = 1,
    ...
    oxwm.bar.block.shell({
        format = "{}",
        command = [[sh -c 'wpctl get-volume @DEFAULT_AUDIO_SINK@ | awk "{ if (\$3 == \"[MUTED]\") print \"VOL MUTE\"; else printf \"VOL %d%%\\n\", \$2 * 100 }"']],
        interval = 1,
        color = colors.cyan,
        underline = true,
    }),
    ...
    oxwm.bar.block.shell({
        format = "{}",
        command = "sh -c 'echo BRI $(( $(brightnessctl g) * 100 / $(brightnessctl m) ))%'",
        interval = 1,
        color = colors.red,
        underline = true,
    }),
    ...
    oxwm.bar.block.shell({
        format = "{}",
        command = "sh -c 'echo Linux $(uname -r)'",
        interval = 1,
        color = colors.red,
        underline = true,
    }),
...
oxwm.autostart("feh --bg-scale /home/thomas/Bilder/walls/landscapes/vally.jpg")
